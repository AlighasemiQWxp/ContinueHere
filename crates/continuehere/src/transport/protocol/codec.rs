...
use super::{ApplicationHello, ProtocolEnvelope, ProtocolLimits, ProtocolMessage, TransportError};
...
fn validate_request_id(request_id: u64, message: &ProtocolMessage) -> Result<(), TransportError> {
    let requires_identifier =
        matches!(message, ProtocolMessage::Ping(_) | ProtocolMessage::Pong(_));
...
mod tests {
    ...
    fn hello_round_trip_is_deterministic() {
        ...
        let envelope = ProtocolEnvelope::hello(
            ApplicationHello::local(&identity).expect("hello should be created"),
        );
